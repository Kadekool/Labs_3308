...
$(document).ready(function() {
  ...
  var url ='https://api.weatherstack.com/forecast?access_key=5bc82451636190abd9d7afe6fe9b20b5&query='+ latitude + ','+ longitude+'&forecast_days=6';
  ...
  $.ajax({url:url, dataType:"jsonp"}).then(function(data) {
    ...
    */
    ...
    console.log(data)
    const current_time = new Date(data.location.localtime);
    ...
    const currentFTemp = getFarenheitTemp(data.current.temperature);

    document.getElementById("temp_today").innerHTML = "" + currentFTemp + "F";

    document.getElementById("thermometer_inner").style.height = currentFTemp + "%";
    
    let color = 'gray';

    if (currentFTemp > 85) {
      color = 'red';
    }else if (currentFTemp < 65){
      color = 'blue';
    }

    document.getElementById("thermometer_inner").style.backgroundColor = color;
  ...
  Object.keys(data.forecast).forEach((key) => {
    console.log(key)
    const forecast_data = data.forecast[key];
    console.log(forecast_data);
    forecastCard += "<div style='width: 20%;'> <div class='card'><div class='card-body'><h5 class='card-title'>"+getKeyTwo(new Date(key))+"</h5> <p class='card-text'>High:"+getFarenheitTemp(forecast_data.maxtemp)+"<br> Low: "+ getFarenheitTemp(forecast_data.mintemp)+"<br> Sunrise:"+forecast_data.astro.sunrise+"<br> Sunset: "+forecast_data.astro.sunset+" </p> </div> </div> </div>";
  })
